                    ...
                    <div class="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                        <table class="min-w-full text-sm text-left text-gray-600 dark:text-gray-300">
                            ...
                            <tbody>
                                ...
                                {{-- PERBAIKAN: KITA CARI DATANYA DI SINI (PALING ATAS) --}}
                                @php
                                    $app = $student->latestInternshipApplication ?? null;
                                    $st = $app ? ($app->status ?? 'pending') : 'no_application';
                                    
                                    // Logika: Jika sekolah di profil kosong, ambil dari form lamaran PKL
                                    $namaSekolah = $student->school ?: ($app->school ?? '-');
                                @endphp
                                ...
                                <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-4 py-4 flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            {{-- Tampilkan nama sekolah di bawah nama siswa --}}
                                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $namaSekolah }}</div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">{{ $student->nim ?? $student->nis ?? '-' }}</td>
                                    
                                    {{-- Tampilkan nama sekolah di kolom Asal Sekolah --}}
                                    <td class="px-4 py-4">{{ $namaSekolah }}</td>
